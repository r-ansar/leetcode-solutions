# Tree Problems

## Concepts Covered
- Tree

## Problems
| Problem Name | File | Difficulty | Notes |
|-------------|------|------------|-------|
| Invert Binary Tree | 01_InvertBinaryTree.cpp | Easy | DFS |
| Maximum Depth Of Binary Tree | 02_MaximumDepthOfBinaryTree.cpp | Easy | DFS |
| Same Binary Tree | 03_SameBinaryTree.cpp | Easy | DFS |


| Binary Tree Level Order Traversal | 06_BinaryTreeLevelOrderTraversal.cpp | Medium | BFS |

## Optional
- https://neetcode.io/problems/invert-a-binary-tree/question?list=blind75
- https://neetcode.io/problems/depth-of-binary-tree/question?list=blind75
- https://neetcode.io/problems/same-binary-tree/question?list=blind75
- https://neetcode.io/problems/level-order-traversal-of-binary-tree/question?list=blind75